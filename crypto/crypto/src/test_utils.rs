...
#[cfg(any(test, feature = "fuzzing"))]
static TEST_DIEM_CRYPTO_SEED: crate::OnceCell<[u8; 32]> = crate::OnceCell::new();

#[cfg(any(test, feature = "fuzzing"))]
impl TestDiemCryptoHasher {
    fn new() -> Self {
        let name = crate::serde_name::trace_name::<TestDiemCrypto>()
...
#[cfg(any(test, feature = "fuzzing"))]
static TEST_DIEM_CRYPTO_HASHER: crate::Lazy<TestDiemCryptoHasher> =
    crate::Lazy::new(TestDiemCryptoHasher::new);
...
impl crate::hash::CryptoHash for TestDiemCrypto {
    ...
    fn hash(&self) -> crate::hash::HashValue {
        ...
        let mut buffer = vec![];
        bcs::serialize_into(&mut buffer, &self)
            .expect("BCS serialization of TestDiemCrypto should not fail");
        let mut state = Self::Hasher::default();
        state
            .write(&buffer)
            .expect("Hash calculation of TestDiemCrypto should not fail");
        state.finish()
    }
}
