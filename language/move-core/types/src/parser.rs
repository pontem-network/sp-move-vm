...
impl<I: Iterator<Item = Token>> Parser<I> {
    ...
    fn parse_transaction_argument(&mut self) -> Result<TransactionArgument> {
        Ok(match self.next()? {
            Token::U8(s) => TransactionArgument::U8(s.parse().map_err(Error::msg)?),
            Token::U64(s) => TransactionArgument::U64(s.parse().map_err(Error::msg)?),
            Token::U128(s) => TransactionArgument::U128(s.parse().map_err(Error::msg)?),
