script {
    ...
    fun test_balance(addr_1: &signer, addr_2: &signer, init_usdt: u128, init_pont: u128, init_btc: u128) {
        ...
        assert(Account::get_native_balance<PONT::T>(addr_1) == init_pont, 2);
        ...
        let pont_1 = Account::deposit_native<PONT::T>(addr_1, 1);
        let pont_2 = Account::deposit_native<PONT::T>(addr_1, 1);
        let pont_3 = Account::deposit_native<PONT::T>(addr_1, 1);

        let pont_1 = Pontem::join(pont_1, pont_3);

        Account::withdraw_native<PONT::T>(addr_2, pont_1);
        Account::withdraw_native<PONT::T>(addr_2, pont_2);
        assert(Account::get_native_balance<PONT::T>(addr_1) == init_pont - 3, 6);
        assert(Account::get_native_balance<PONT::T>(addr_2) == 3, 6);
    }
}